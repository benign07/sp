...
class DeveloperUi:
  ...
  def get_v_rel(lead_status: bool, lead_v_rel: float, is_metric: bool) -> UiElement:
    """Relative velocity vs primary lead car."""
    ...
    if lead_status:
      if lead_v_rel < -4.4704:  # Approaching faster than 10mph
        color = rl.RED
      elif lead_v_rel < 0:
        color = rl.Color(255, 188, 0, 255)  # Orange
  ...
  def get_steering_angle_deg(angle_steers: float, lat_active: bool, steer_override: bool) -> UiElement:
    ...
    # Red if large steering angle, orange if moderate
...
class DeveloperUiRenderer(Widget):
  ...
  def _draw_right_dev_ui(self, rect: rl.Rectangle) -> None:
    ...
    # rl.draw_rectangle(x - 10, y - 10, container_width + 20, container_height + 10,
    #                   rl.Color(0, 0, 0, 100))
    ...
    # Add steering angle or lateral accel based on control type
  ...
  def _draw_right_dev_ui_element(self, x: int, y: int, element: UiElement) -> int:
    ...
    if element.units:
      x += 120
      y -= 25
      # Note: Raylib doesn't have native text rotation, so we'll draw it horizontally
      # For true rotation, you'd need to render to a texture first
      unit_size = 28
  ...
  def _draw_bottom_dev_ui(self, rect: rl.Rectangle) -> None:
    ...
    if sm['controlsState'].lateralControlState.which() == 'torqueState':
      ...
      if sm.valid['gpsLocationExternal'] or sm.valid['gpsLocation']:
        gps_source = 'gpsLocationExternal' if sm.valid['gpsLocationExternal'] else 'gpsLocation'
        gps = sm[gps_source]
        gps_data = gps.gpsLocationExternal if gps_source == 'gpsLocationExternal' else gps.gpsLocation
    ...
    if sm.valid['gpsLocationExternal'] or sm.valid['gpsLocation']:
      ...
      gps = sm[gps_source]
      gps_data = gps.gpsLocationExternal if gps_source == 'gpsLocationExternal' else gps.gpsLocation
      gps_accuracy = gps_data.horizontalAccuracy if gps_source == 'gpsLocationExternal' else 1.0
      elements.append(DeveloperUi.get_altitude(gps_accuracy, gps_data.altitude))
